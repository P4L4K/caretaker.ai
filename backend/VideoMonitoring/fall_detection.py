...
class FallDetector:
    def __init__(
        ...
        *,
        ...
        if not os.path.exists(model_name):
            ...
            parent_model_path = os.path.join(parent_dir, model_name)
            if os.path.exists(parent_model_path):
                model_name = parent_model_path
                print(f"[FallDetector] Found model in parent directory: {model_name}")
        ...
        print(f"[FallDetector] Using device: {self.device}")
        ...
        if self.use_half:
            try:
                ...
                print("[FallDetector] Half precision (FP16) enabled for faster inference")
            except Exception as e:
                print(f"[FallDetector] Could not enable FP16: {e}")
                self.use_half = False
        
        # Optimized image size for real-time processing
        self.imgsz = 416  # Smaller than 640 for faster inference
    ...
    def detect_fall(self, frame: np.ndarray) -> Dict[str, object]:
        ...
        try:
            results = self.model.predict(frame, conf=self.conf, imgsz=self.imgsz, verbose=False, device=self.device)
